...
async def persist_message_atomic(
    ...
    if not USE_SUPABASE_DB:
        print("[Persist] SKIP: Supabase not configured")
        return None

    try:
        supabase = await get_supabase_client()
        if not supabase:
            print("[Persist] SKIP: No Supabase client")
            return None
        ...
        if result.data and len(result.data) > 0:
            row = result.data[0]
            print(
                f"[Persist] OK: thread={thread_id[:8]}... msg={message_id[:8]}... seq={row.get('seq')} role={role}"
            )
            return row

        print(
            f"[Persist] FAIL: No result from RPC (thread={thread_id[:8]}..., msg={message_id[:8]}...)"
        )
        return None
...
async def persist_message(message: Any, config: RunnableConfig, index: int = 0) -> bool:
    ...
    if not message_data:
        print("[Persist] SKIPPED: Cannot serialize message")
        return False

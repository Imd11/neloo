...
class MessagePersistenceMiddleware(AgentMiddleware):
    ...
    async def abefore_agent(self, state: Dict[str, Any], runtime: Any) -> Dict[str, Any] | None:
        ...
        if not thread_id or thread_id == 'default':
            print(f"[PersistMiddleware] SKIP human: No valid thread_id")
            return None
    ...
    async def _persist_human_message(
        ...
        if not msg_id or not isinstance(msg_id, str) or not msg_id.strip():
            print(f"[PersistMiddleware] ERROR: Human message has no stable id - frontend must provide id")
            print(f"  thread_id={thread_id[:8]}..., run_id={run_id[:8]}...")
            return
        
        # Serialize message
        message_data = serialize_message(message)
        if not message_data:
            print(f"[PersistMiddleware] SKIP: Cannot serialize human message")
            return
    ...
    async def awrap_model_call(
        ...
        if not thread_id or thread_id == 'default':
            print(f"[PersistMiddleware] SKIP AI: No valid thread_id")
            return response
        
        # Persist each AI message
        for i, msg in enumerate(ai_messages):
            if isinstance(msg, AIMessage) or (hasattr(msg, 'type') and getattr(msg, 'type', '') == 'ai'):
                try:
    ...
    async def _persist_ai_message(
        ...
        message_data = serialize_message(message)
        if not message_data:
            print(f"[PersistMiddleware] SKIP: Cannot serialize AI message")
            return

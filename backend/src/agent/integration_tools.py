...
def _generate_legacy_idempotency_key(thread_id: str, run_id: str, action: str, params: dict) -> str:
    """
...
async def integrations_list_actions(
    ...
    # Resolve user_id
    user_id = _resolve_user_id_from_config(config)
    _resolve_thread_id_from_config(config) or thread_id_ctx.get()
...
async def _invoke_allowed_action_legacy(
    ...
    try:
        ...
        if not tool_instance:
            ...
            print(
                f"[integrations_execute] Composio returned {len(available_actions)} actions for {app_name}:"
            )
    ...
    try:
        ...
        if isinstance(result, dict):
            ...
            result_url = (
                result.get("url") or result.get("link") or result.get("data", {}).get("url")
            )
            ...
            if any(kw in result.lower() for kw in error_keywords):
                ...
                print(
                    f"[integrations_execute] Result string appears to be an error: {result[:100]}"
                )
        ...
        print(
            f"[integrations_execute] Success: result_id={result_id}, result_url={result_url}, status={execution_status}"
        )
    ...
    return {
        ...
        "result_id": result_id,
        "result_url": result_url,
        "message": f"{app_name} action executed successfully"
        + (f" - View: {result_url}" if result_url else ""),
    }
...
async def _invoke_allowed_action(
    ...
    if classification in {"write", "sensitive"}:
        ...
        try:
            ...
            return {
                "status": "error",
                "message": "Action reservation failed; nothing was executed.",
            }

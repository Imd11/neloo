...
class CreateTriggerRequest(BaseModel):
    ...
    default_prompt: Optional[str] = Field(
        default=None, description="Default prompt to send when triggered"
    )
...
async def list_triggers(user: dict = Depends(get_persistent_user)):
    ...
    result = (
        supabase.table("scheduled_triggers")
...
async def update_trigger(
    trigger_id: str, request: UpdateTriggerRequest, user: dict = Depends(get_persistent_user)
):
...
async def get_trigger_logs(
    trigger_id: str, limit: int = 20, user: dict = Depends(get_persistent_user)
):
    ...
    result = (
        supabase.table("trigger_execution_logs")
        .select("*")

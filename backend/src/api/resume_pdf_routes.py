...
async def export_pdf(
    request: PDFRequest, user: dict = Depends(get_current_user)
) -> StreamingResponse:
    """
    ...
    if len(request.html) > 500 * 1024:
        raise HTTPException(status_code=400, detail="HTML content too large (max 500KB)")
    ...
    try:
        ...
        pdf_bytes = await asyncio.wait_for(generate_pdf_with_playwright(request.html), timeout=30.0)
        ...
        raise HTTPException(status_code=504, detail="PDF generation timed out (30s limit)")
...
async def pdf_health_check():
    ...
    try:
        ...
        return {"status": "healthy", "playwright": "available", "chromium": "installed"}
    except ImportError:
        return {"status": "degraded", "playwright": "not installed", "chromium": "unknown"}
    except Exception as e:
        return {
            "status": "unhealthy",

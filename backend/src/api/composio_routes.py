...
async def connect_app(request: ConnectRequest, user: dict = Depends(get_persistent_user)):
    """
...
async def oauth_callback(
    ...
    if not supabase:
        return RedirectResponse(
            f"{FRONTEND_URL}/?openSettings=1&tab=apps&error=db_error", status_code=302
        )
    ...
    if not result.data or len(result.data) == 0:
        ...
        return RedirectResponse(
            f"{FRONTEND_URL}/?openSettings=1&tab=apps&error=invalid_state", status_code=302
        )
    ...
    try:
        ...
        composio_user_id = getattr(connection, "user_id", None) or getattr(
            connection, "entity_id", None
        )
        if composio_user_id and str(composio_user_id) != str(user_id):
            print(f"[OAuth Callback] User mismatch: {composio_user_id} != {user_id}")
            return RedirectResponse(
                f"{FRONTEND_URL}/?openSettings=1&tab=apps&error=user_mismatch", status_code=302
            )
    ...
    return RedirectResponse(
        f"{FRONTEND_URL}/?openSettings=1&tab=apps&app={app_name}&status=success", status_code=302
    )


@router.get("/status/{app_name}", response_model=StatusResponse)
async def check_status(app_name: str, user: dict = Depends(get_persistent_user)):
    """
...
async def disconnect_app(request: DisconnectRequest, user: dict = Depends(get_persistent_user)):
    """
    Disconnect an app.
    ...
    await (
        supabase.table("user_integrations")
        .update({"status": "disconnected", "updated_at": datetime.now(timezone.utc).isoformat()})
        .eq("user_id", user_id)
        .eq("app_name", app_name)
        .execute()
    )
...
async def get_available_apps():
    ...
    """
    return {"apps": list(get_app_auth_configs().keys())}

...
- Shared rate limiting is built in for chat runs, image routes, Slides, translation, resume AI, E2B, Connected Apps, and guest-session issuance. Set the same server-only `RATE_LIMIT_REDIS_URL` and `RATE_LIMIT_NAMESPACE` in Railway and Vercel. Vercel deployments use the platform-overwritten `x-vercel-forwarded-for` header. For other production proxies, set `TRUSTED_PROXY_HOPS` to the exact number of trusted reverse proxies and configure the outer proxy to strip and replace incoming forwarding headers. Requests fail closed when no trusted client address is available.
- `GUEST_SESSIONS_PER_DAY` limits how many new anonymous identities one trusted client address can mint in 24 hours; it defaults to `2`. Existing guest sessions remain valid for their normal lifetime.

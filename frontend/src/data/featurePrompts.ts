...
export function getHumanizePrompt(
  templateId: number | null,
  instruction?: string
): string {
  const templateInstruction =
    instruction ||
    (templateId ? HUMANIZE_FALLBACK_INSTRUCTIONS[templateId] : undefined);
...
export function getPromptOptimizePrompt(
  templateId: number | null,
  instruction?: string
): string {
  const templateInstruction =
    instruction ||
    (templateId
      ? PROMPT_OPTIMIZE_FALLBACK_INSTRUCTIONS[templateId]
      : undefined);

...
async function request<T>(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Slides persistence failed: ${response.status} - ${errorText}`
    );
  }
...
export const savePresentation = async (
  ...
  return request<PresentationData>(
...
export const getPresentation = async (
  ...
  try {
    return await request<PresentationData>(
      `/api/slides/presentations/${encodeURIComponent(id)}`,
      undefined,
      accessToken
    );
  } catch (error) {

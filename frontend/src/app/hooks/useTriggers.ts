...
export function useTriggers(agentId?: string) {
  ...
  const createTrigger = useCallback(
    async (data: CreateTriggerData): Promise<Trigger | null> => {
  ...
  const toggleTrigger = useCallback(
    async (triggerId: string, enabled: boolean): Promise<boolean> => {
  ...
  const getExecutionLogs = useCallback(
    async (triggerId: string): Promise<ExecutionLog[]> => {
      if (authLoading || !accessToken) return [];
      try {
        const baseUrl = getBaseUrl();
        const headers = buildBearerHeaders(accessToken);
        ...
        const response = await fetch(
          `${baseUrl}/api/triggers/${triggerId}/logs`,
          { headers }
        );
  ...
  const runTrigger = useCallback(
    async (
      ...
      try {
        ...
        const response = await fetch(
          `${baseUrl}/api/triggers/${triggerId}/run`,
          {
            method: "POST",
            headers,
          }
...
export const CRON_PRESETS = [
  ...
  { label: "每天早上9点", value: "0 9 * * *", description: "每天上午9:00执行" },
  {
    label: "每天晚上8点",
    value: "0 20 * * *",
    description: "每天晚上8:00执行",
  },
  {
    label: "工作日早上9点",
    value: "0 9 * * 1-5",
    description: "周一至周五上午9:00执行",
  },
  {
    label: "每周一早上9点",
    value: "0 9 * * 1",
    description: "每周一上午9:00执行",
  },

...
const HEADLINE_KEYS = [
  "translate.headline_1",
  "translate.headline_2",
  "translate.headline_3",
];
...
export function TranslatePanel({ onBack, modelId }: TranslatePanelProps) {
  ...
  return (
    <div className="relative mx-auto flex h-full max-w-5xl flex-col items-center justify-center px-4 py-8">
      {/* Back Button - Fixed position, below TopBar and to the right of sidebar */}
      <button
        onClick={onBack}
        className="fixed top-14 z-10 rounded-lg p-2 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
      ...
      {/* Headline */}
      <h1
        className="mb-8 text-center text-3xl font-medium text-foreground fade-in md:text-4xl"
      ...
      <div className="mb-6 grid w-full grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <div className="relative">
          <Textarea
            ...
            className="min-h-[240px] resize-none rounded-xl border-border bg-muted/50 p-4 text-base"
          />
        </div>
      ...
      <div className="mb-6 flex w-full flex-wrap justify-center gap-3">
        {STYLE_CODES.map((style) => (
          <button
            ...
            className={cn(
              "flex flex-col items-center rounded-xl border px-4 py-3 transition-all",
              "hover:bg-muted/50",
              selectedStyle === style
          ...
          >
            <span className="text-sm font-medium text-foreground">
              {t(`translate.styles.${style}.label`)}
            </span>
            <span className="mt-0.5 text-xs text-muted-foreground">
              {t(`translate.styles.${style}.description`)}
            </span>
          </button>
        ))}
      </div>
      ...
      <Button
        ...
        disabled={
          loading || authLoading || !session.access_token || !sourceText.trim()
        }
        className="bg-foreground px-12 py-4 text-lg text-background hover:bg-foreground/90 dark:bg-foreground dark:text-background dark:hover:bg-foreground/90"
      >
        {loading ? (
          <>

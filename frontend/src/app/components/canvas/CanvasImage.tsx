...
import { ContextMenu, ContextMenuItem } from "@/app/components/ui/custom-context-menu";
...
export const CanvasImage = forwardRef<CanvasImageRef, CanvasImageProps>(({
    ...
    const [downloadStatus, setDownloadStatus] = useState<{ visible: boolean; status: "downloading" | "success" | "error" }>({ visible: false, status: "downloading" });
    ...
    useEffect(() => {
        if (isEditing && imageRef.current && visualCanvasRef.current && dataCanvasRef.current) {
            const width = imageRef.current.naturalWidth;
            const height = imageRef.current.naturalHeight;
    ...
    const draw = (e: React.MouseEvent) => {
        if (!isEditing || !visualCanvasRef.current || !dataCanvasRef.current) return;
        ...
        if (!isEraser && visualCanvasRef.current && dataCanvasRef.current) {
            ...
            if (vCtx2) {
                vCtx2.clearRect(0, 0, visualCanvasRef.current.width, visualCanvasRef.current.height);
                ...
                vCtx2.fillRect(0, 0, visualCanvasRef.current.width, visualCanvasRef.current.height);
            }
        }
    };
    ...
    const stopDrawing = () => {
        if (isRectTool && rectStart && currentRect) {
            ...
            if (dCtx && vCtx && visualCanvasRef.current && dataCanvasRef.current) {
                ...
                dCtx.fillRect(currentRect.x, currentRect.y, currentRect.width, currentRect.height);

                vCtx.clearRect(0, 0, visualCanvasRef.current.width, visualCanvasRef.current.height);
                vCtx.globalCompositeOperation = "source-over";
                vCtx.drawImage(dataCanvasRef.current, 0, 0);
                vCtx.globalCompositeOperation = "source-in";
                vCtx.fillStyle = "rgba(255, 64, 64, 0.25)";
                vCtx.fillRect(0, 0, visualCanvasRef.current.width, visualCanvasRef.current.height);
            }
            setRectStart(null);
            setCurrentRect(null);
        } else {
    ...
    return (
        ...
        >
                    ...
                    <div className={`relative group rounded-xl overflow-hidden transition-all duration-300 ${isSelected || isEditing || isMenuOpen
                                ...
                                <div className="relative flex flex-col items-center gap-4 px-6 text-center">
                                    ...
                                    {onRegenerate && (
                                        <button
                                            ...
                                            className="mt-2 px-6 py-2.5 bg-gradient-to-r from-red-600 to-red-500 hover:from-red-500 hover:to-red-400 text-white font-medium rounded-lg transition-all duration-200 flex items-center gap-2 shadow-lg shadow-red-500/20"
                                        >
                            ...
                            >
                                ...
                                <div className="relative flex flex-col items-center gap-4">
                                    <div className="relative w-16 h-16">
                                        <div className="absolute inset-0 rounded-full border-2 border-cyan-500/30 border-t-cyan-400 animate-[spin_3s_linear_infinite]" />
                                        <div className="absolute inset-2 rounded-full border-2 border-violet-500/30 border-t-violet-400 animate-[spin_1.5s_linear_infinite_reverse]" />
                                        <div className="absolute inset-0 flex items-center justify-center">
                                            <div className="w-3 h-3 rounded-full bg-gradient-to-r from-cyan-400 to-violet-400 animate-pulse shadow-[0_0_20px_rgba(34,211,238,0.5)]" />
                                        </div>
                        ...
                        {isEditing && (
                            <>
                                ...
                                />
                                {showCursor && !isRectTool && typeof document !== 'undefined' && createPortal(
                                    <div
                                        className="fixed pointer-events-none rounded-full border border-white/50 bg-red-500/20 z-[9999] transform -translate-x-1/2 -translate-y-1/2"
                                        style={{
                ...
                {!isEditing && (
                    <>
                        <ContextMenuItem onClick={() => onEditStart(id, "inpainting")}>
                            <PenTool className="w-4 h-4 mr-2" /> {t("canvas.ai_edit_image")}
                        </ContextMenuItem>
                        <ContextMenuItem onClick={() => onEditStart(id, "text")}>
                            <TypeIcon className="w-4 h-4 mr-2" /> {t("canvas.ai_edit_text")}
                        </ContextMenuItem>

                        {!parentId && onRegenerate && (
                            <ContextMenuItem onClick={() => onRegenerate(id)}>
                                <RotateCcw className="w-4 h-4 mr-2" /> {t("canvas.regenerate")}
                            </ContextMenuItem>
                        )}
                        ...
                        <ContextMenuItem onClick={async () => {
                            ...
                            try {
                                ...
                                setTimeout(() => setDownloadStatus({ visible: false, status: "downloading" }), 2000);
                            }
                        }}>
                            <Download className="w-4 h-4 mr-2" /> {t("canvas.download_image")}
                        </ContextMenuItem>

                        {parentId && onDelete && (
                            <ContextMenuItem onClick={() => onDelete(id)}>
                                <X className="w-4 h-4 mr-2" /> {t("canvas.delete_image")}
                            </ContextMenuItem>
                        )}
                    </>
                )}
            ...
            <DownloadOverlay
                ...
                onComplete={() => setDownloadStatus({ visible: false, status: "downloading" })}
            />
        </div>
    );
});

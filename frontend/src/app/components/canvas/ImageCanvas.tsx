...
import {
  CanvasImage as CanvasImageCard,
  type CanvasImageRef,
} from "./CanvasImage";
...
export function ImageCanvas({
  ...
  const [editTool, setEditTool] = useState<"brush" | "eraser" | "rect">(
    "brush"
  );
  ...
  const getBatchImages = useCallback(
    (source: CanvasImage[], rootId: string) => {
      return source.filter(
        (candidate) => getRootImage(source, candidate.id)?.id === rootId
      );
    },
    [getRootImage]
  );

  const getNextGeneratedPlacement = useCallback(
    (source: CanvasImage[], imageHeight: number) => {
      ...
      const nextRowTop =
        Math.max(
          ...roots.map((root) => root.y + getImageDimensions(root).height)
        ) + ROW_GAP;
  ...
  const getNextEditPlacement = useCallback(
    (source: CanvasImage[], parentId: string, parentDisplayHeight: number) => {
  ...
  useEffect(() => {
    if (flyingImage && onFlyingComplete) {
      const timer = setTimeout(() => {
        const imageHeight = getCanvasImageHeight(
          flyingImage.generationParams?.size
        );
        onImagesChange((prev) => {
  ...
  const handleEditConfirm = async () => {
    ...
    try {
      ...
      const editGenerationParams: CanvasImage["generationParams"] = {
        ...
        resolution:
          editResolution || editingSource?.generationParams?.resolution || "1k",
        ...(editSize
          ? { size: editSize }
      ...
      onImagesChange((prev) => {
        const nextPlacement = getNextEditPlacement(
          prev,
          editingImageId,
          parentDisplayHeight
        );
          ...
          prev.map((img) =>
            img.id === placeholderId
              ? {
                  ...
                  error: "AI edit failed: no image returned",
                }
              : img
          )
        );
  ...
  const handleRegenerate = async (imageId: string) => {
    ...
    onImagesChange(
      images.map((item) =>
        item.id === imageId
          ? { ...item, url: "", loadingType: "generate" as const }
          : item
      )
    );
    ...
    try {
      const response = await fetch("/api/generate-image", {
        ...
        body: JSON.stringify({
          ...
          ...(img.generationParams.size
            ? { size: img.generationParams.size }
            : {}),
          ...(img.generationParams.modelId
            ? { model: img.generationParams.modelId }
            : {}),
        }),
      });
  ...
  const handleDelete = (imageId: string) => {
    ...
    if (toDelete.length > 1) {
      if (
        !confirm(t("canvas.confirm_delete_images", { count: toDelete.length }))
      ) {
        return;
      }
    }
  ...
  return (
    <div className="relative flex h-full w-full flex-col overflow-hidden bg-canvas-bg">
      {/* Top Bar */}
      <CanvasTopBar
        activeTool={tool}
      ...
      <div
        ...
        style={{
          ...
          backgroundImage:
            "radial-gradient(hsl(var(--canvas-dots)) 1px, transparent 1px)",
        ...
        >
          ...
          <svg className="pointer-events-none absolute inset-0 h-full w-full overflow-visible">
            {images.map((img) => {
              ...
              const bendDirection =
                y1 === y2 ? stableBendDirection : y2 > y1 ? 1 : -1;
              const bendAmount = Math.min(
                48,
                Math.max(16, Math.abs(x2 - x1) * 0.1)
              );
            ...
            <motion.div
              ...
              animate={{
                left:
                  ...
                  viewOffset.x +
                  flyingTarget.x * scale,
                top:
                  canvasRef.current.getBoundingClientRect().top +
                  viewOffset.y +
                  flyingTarget.y * scale,
      ...
      {editingImageId && (
        <div className="absolute bottom-8 left-1/2 z-50 flex -translate-x-1/2 items-center gap-4 rounded-full border border-canvas-border bg-canvas-topbar/95 px-5 py-2.5 text-canvas-foreground shadow-2xl backdrop-blur-xl animate-in fade-in slide-in-from-bottom-4">
          <div className="flex items-center gap-1.5 border-r border-canvas-border pr-4">
            ...
            {editTool !== "rect" && (
              <div className="relative flex items-center">
                {isSliderDragging && (
                  <div
                    className="pointer-events-none absolute left-1/2 z-50 flex -translate-x-1/2 flex-col items-center gap-1"
                    style={{ bottom: "100%", marginBottom: "8px" }}
                  >
                    <div className="rounded-lg border border-canvas-border bg-canvas-topbar/95 p-3 shadow-xl backdrop-blur-sm">
                      <div
                        className="rounded-full border-2 border-red-400 bg-red-500/50"
                        style={{
          ...
          <div className="flex items-center gap-2 border-r border-canvas-border pr-4">
            <Input
              className="h-9 w-64 border-none !bg-transparent text-canvas-foreground placeholder:text-canvas-muted focus-visible:ring-0"
              placeholder={
                editMode === "inpainting"
                  ? t("canvas.edit_placeholder")
                  : t("canvas.edit_text_placeholder")
              }
              value={editPrompt}
          ...
          <div className="flex items-center gap-2">
            <Button
              size="icon"
              variant="ghost"
              className="h-9 w-9 rounded-full text-canvas-muted hover:bg-red-500/15 hover:text-red-500"
            ...
            <Button
              size="icon"
              className="h-9 w-9 rounded-full bg-blue-500 text-white shadow-md shadow-blue-500/20 hover:bg-blue-600"

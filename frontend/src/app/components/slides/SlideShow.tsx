...
import {
  generateSlideImage,
  generateSingleSlide,
} from "@/app/slides/lib/slidesService";
import {
  savePresentation,
  updateSlideImage,
  updateSlideContent,
...
const SlideShow: React.FC<SlideShowProps> = ({
  ...
  const triggerGeneration = useCallback(
    async (slideIndex: number, force = false) => {
      ...
      if (!force) {
        if (
          ...
          generatedRef.current.has(slide.id)
        )
          return;
      }
            ...
            objects.forEach((obj) => {
              ...
              if (obj.type === "text" && obj.text) {
                ...
                pptxSlide.addText(obj.text, {
                  ...
                  underline: obj.textDecoration?.includes("underline")
                    ? { style: "sng" }
                    : undefined,
  ...
  const handleConfirmAddSlide = async (description: string) => {
    ...
    try {
      const newSlideData = await generateSingleSlide(
        topic,
        description,
        slides,
        newSlideIndex
  ...
  return (
    <div className="relative flex h-full flex-col bg-black text-white">
      {/* Navbar */}
      <div className="relative z-20 flex h-16 flex-shrink-0 items-center justify-between border-b border-zinc-800 bg-zinc-900/50 px-6">
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="rounded-full p-2 text-zinc-400 transition hover:bg-zinc-800 hover:text-white disabled:opacity-30 disabled:hover:bg-transparent"
            title="撤销"
          >
            <Undo size={18} />
          </button>
        ...
        <div className="absolute left-1/2 top-1/2 flex -translate-x-1/2 -translate-y-1/2 transform flex-col items-center">
          <h1 className="max-w-md truncate text-sm font-semibold text-zinc-200">
            {topic}
          </h1>
        ...
        <div className="flex items-center gap-4">
          {generatingCount > 0 && (
            <span className="flex animate-pulse items-center gap-2 text-xs text-zinc-500">
              <Loader2
                size={12}
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-lg bg-white px-4 py-2 text-sm font-bold text-black transition hover:bg-zinc-200 disabled:opacity-50"
          >
            {isExporting ? (
              <Loader2
                size={16}

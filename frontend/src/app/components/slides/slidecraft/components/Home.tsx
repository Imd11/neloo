...
const Home: React.FC<HomeProps> = ({
  ...
  return (
    <div
      className="flex min-h-screen flex-col items-center p-6 pt-12 md:pt-16"
      onDragOver={(e) => {
        e.preventDefault();
        setIsDragging(true);
      }}
    ...
    >
      ...
      {isDragging && (
        <div className="border-purple-500 bg-purple-500/10 fixed inset-0 z-50 flex items-center justify-center border-2 border-dashed">
          <p className="text-purple-300 text-xl font-medium">松开以添加文件</p>
        </div>
      )}

      <div className="mb-8 text-center fade-in">
        <div className="border-purple-500/20 bg-purple-500/10 text-purple-400 mb-5 inline-flex items-center gap-2 rounded-full border px-4 py-1.5 text-sm">
          <Sparkles size={14} />
      ...
      <div
        className="mb-4 w-full max-w-2xl fade-in"
      ...
      >
        <div className="group relative">
          <input
            ...
            className="focus:border-purple-500/50 w-full rounded-2xl border border-zinc-800 bg-zinc-900 px-6 py-5 pr-28 text-lg text-white placeholder-zinc-500 transition-all focus:outline-none focus:ring-2 focus:ring-purple-500/20"
          />
          <div className="absolute right-2 top-1/2 flex -translate-y-1/2 items-center gap-2">
            ...
            <button
              ...
              disabled={!topic.trim()}
              className="bg-purple-600 hover:bg-purple-500 flex items-center gap-2 rounded-xl px-5 py-2.5 font-medium text-white shadow-lg shadow-purple-900/30 transition-all disabled:bg-zinc-800 disabled:text-zinc-600 disabled:shadow-none"
            >
              <span>开始</span>
              <ArrowRight size={16} />
            </button>
      ...
      <div className="w-full max-w-5xl">
        <h2 className="mb-4 flex items-center gap-2 text-sm font-medium uppercase tracking-wider text-zinc-400">
          模板风格
          {selectedPresetId && (
            <span className="bg-purple-500/10 text-purple-300 rounded-full px-2 py-0.5 text-[10px]">
              已选{" "}
              {PRESETS.find((item) => item.id === selectedPresetId)?.nameZh ||
                selectedPresetId}
            </span>
          )}
          {!selectedPresetId && recommendedId && (
            <span className="rounded-full bg-amber-500/10 px-2 py-0.5 text-[10px] text-amber-300">
              推荐{" "}
              {PRESETS.find((item) => item.id === recommendedId)?.nameZh ||
                recommendedId}
            </span>
          )}
        </h2>
          ...
          {PRESETS.map((preset) => {
            ...
            return (
              <button
                ...
                onClick={() =>
                  setSelectedPresetId(
                    preset.id === selectedPresetId ? null : preset.id
                  )
                }
      ...
      {history.length > 0 && (
        <div
          className="mt-14 w-full max-w-4xl fade-in"
            ...
            {history.map((presentation) => (
              ...
              >
                ...
                <button
                  ...
                  className="absolute right-2 top-2 rounded-lg bg-zinc-800 p-1.5 text-zinc-500 opacity-0 transition hover:text-red-400 group-hover:opacity-100"
                >
                  <Trash2 size={14} />
                </button>
              </div>

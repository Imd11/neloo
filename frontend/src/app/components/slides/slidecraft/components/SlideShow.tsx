...
const SlideShow: React.FC<SlideShowProps> = ({ presentation, style, modelId, accessToken, onBack, onSlidesUpdate }) => {
    ...
    useEffect(() => {
        const generateQueue = async () => {
            ...
            for (const idx of order) {
                ...
                if (!slide || slide.imageBase64 || slide.isGeneratingImage || imageQueueRef.current.has(slide.id)) continue;
    ...
    const generateImageForSlide = async (idx: number, slide: Slide) => {
        updateSlideAtIndex(idx, { isGeneratingImage: true, generationFailed: false });
        ...
        try {
            ...
            updateSlideAtIndex(idx, { imageBase64: base64, isGeneratingImage: false });
        } catch (err: any) {
            if (err.name !== 'AbortError') {
                updateSlideAtIndex(idx, { isGeneratingImage: false, generationFailed: true });
            }
        } finally {
    ...
    const updateSlideAtIndex = useCallback((idx: number, updates: Partial<Slide>) => {
        setSlides(prev => {
            const next = [...prev];
    ...
    const handleRegenerate = (desc: string) => {
        ...
        if (!slide) return;
        imageQueueRef.current.delete(slide.id);
        const updatedSlide = { ...slide, visualDescription: desc, imageBase64: undefined, generationFailed: false };
        const newSlides = [...slides];
    ...
    const handleReorder = (from: number, to: number) => {
        ...
        else if (from < currentIndex && to >= currentIndex) setCurrentIndex(currentIndex - 1);
        else if (from > currentIndex && to <= currentIndex) setCurrentIndex(currentIndex + 1);
    };
        ...
        try {
            ...
            for (const slide of slides) {
                ...
                if (slide.imageBase64) {
                    pptSlide.background = { data: `image/png;base64,${slide.imageBase64}` };
                } else {
                    pptSlide.background = { color: palette.background.replace('#', '') };
                    const textColor = slide.imageBase64 ? 'FFFFFF' : palette.primaryText.replace('#', '');
                    pptSlide.addText(slide.title, {
                        x: 0.8, y: 0.8, w: 8.4,
                        fontSize: 32, fontFace: 'Inter', color: textColor, bold: true,
                        align: 'left', valign: 'top', margin: 0,
                        shadow: { type: 'outer', blur: 3, offset: 1, color: '000000', opacity: 0.5 },
                    });
                    pptSlide.addText(slide.content, {
                        x: 0.8, y: 2.0, w: 8.4, h: 3.0,
                        fontSize: 16, fontFace: 'Inter', color: textColor,
            ...
            // Safe filename
            const safeName = presentation.topic.slice(0, 30).replace(/[\\/:*?"<>|]/g, '_').trim();
    ...
    return (
        <div className="min-h-screen h-screen flex flex-col bg-zinc-950">
            {/* Top bar */}
            <div className="flex items-center justify-between px-4 h-14 border-b border-zinc-800 bg-zinc-950 z-50 flex-shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    <button onClick={handleUndo} disabled={historyIndex <= 0}
                        className="p-1.5 text-zinc-500 hover:text-white hover:bg-zinc-800 rounded-lg disabled:opacity-30 transition" title="撤销 (⌘Z)">
                        <Undo size={16} />
                    </button>
                    <button onClick={handleRedo} disabled={historyIndex >= history.length - 1}
                        className="p-1.5 text-zinc-500 hover:text-white hover:bg-zinc-800 rounded-lg disabled:opacity-30 transition" title="重做 (⌘⇧Z)">
                        <Redo size={16} />
                    </button>
                </div>

                {/* Centered title */}
                <h2 className="absolute left-1/2 transform -translate-x-1/2 text-sm font-medium text-zinc-300 truncate max-w-xs pointer-events-none">
                    {presentation.topic}
                </h2>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-zinc-500 font-mono">{currentIndex + 1} / {slides.length}</span>
                    <button
                        onClick={handleExportPPTX}
                        disabled={isExporting}
                        className="flex items-center gap-1.5 px-4 py-2 bg-white hover:bg-zinc-200 disabled:bg-zinc-800 text-zinc-900 disabled:text-zinc-500 text-sm rounded-xl font-medium transition shadow-lg"
                    >

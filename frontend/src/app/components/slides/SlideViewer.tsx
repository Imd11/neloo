...
const SlideViewer: React.FC<SlideViewerProps> = ({
  ...
  const [editAreaStyle, setEditAreaStyle] =
  ...
  return (
    <div className="relative flex flex-1 flex-col overflow-hidden bg-zinc-900/50">
      <div
        className="relative flex flex-1 items-center justify-center bg-zinc-950 pb-16"
        ref={containerRef}
        onMouseDown={(e) => {
      ...
      >
        <button
          onClick={onPrev}
          disabled={isFirst}
          className="absolute left-4 z-40 rounded-full bg-black/50 p-4 text-white backdrop-blur transition hover:bg-black/80 disabled:opacity-0"
        >
          <ChevronLeft size={24} />
        </button>
        ...
        <div className="relative overflow-hidden rounded border border-zinc-800 shadow-2xl">
                ...
                if (obj.type === "text") {
                  return (
                    <Text
                      ...
                      onDblClick={() =>
                        handleDoubleClick(obj.id, obj.text || "")
                      }
          ...
          {(slide.isGeneratingImage || slide.generationFailed) && (
            <div className="absolute inset-0 z-50 flex flex-col items-center justify-center gap-6 bg-black/80 p-6 text-center backdrop-blur-md">
              {slide.isGeneratingImage && (
                <>
                  <div className="text-purple-400 flex animate-pulse flex-col items-center gap-2">
                    <Loader2
                      size={48}
                  ...
                  <button
                    ...
                    className="mt-4 flex items-center gap-2 rounded-full border border-red-500/50 bg-red-500/20 px-6 py-2 text-sm font-semibold text-red-200 transition hover:bg-red-500/40"
                  >
                    <Square
                      size={16}
                      fill="currentColor"
              ...
              {slide.generationFailed && (
                <div className="flex flex-col items-center gap-4 duration-300 animate-in fade-in zoom-in">
                  ...
                  <div className="mt-2 flex items-center gap-4">
                    <button
                      onClick={onIgnore}
                      className="rounded-full border border-zinc-700 bg-zinc-800 px-6 py-2.5 text-sm font-semibold text-zinc-200 transition hover:border-zinc-600 hover:bg-zinc-700"
                    >
                      忽略
                    </button>
                    <button
                      onClick={onRetry}
                      className="bg-purple-600 hover:bg-purple-500 flex items-center gap-2 rounded-full px-6 py-2.5 text-sm font-semibold text-white shadow-[0_0_15px_rgba(168,85,247,0.4)] transition"
                    >
          ...
          {/* Top Left Controls */}
          <div className="absolute left-4 top-4 z-[60] flex gap-2">
            <button
              ...
              className="rounded-full border border-zinc-700/50 bg-zinc-800/80 p-2 text-white shadow-lg transition hover:bg-red-500"
              title="删除幻灯片"
            >
              <Trash2 size={18} />
            </button>
          ...
          <div className="absolute right-4 top-4 z-[60] flex gap-2">
            {slide.isGeneratingImage ? (
              <button
                ...
                onPointerDown={(e) => e.stopPropagation()}
                onMouseDown={(e) => e.stopPropagation()}
                className="rounded-full bg-red-500/80 p-2 text-white shadow-lg hover:bg-red-500"
              >
                <Square
                  size={18}
                  fill="currentColor"
              ...
              <button
                ...
                onPointerDown={(e) => e.stopPropagation()}
                onMouseDown={(e) => e.stopPropagation()}
                className="hover:bg-purple-500 rounded-full border border-zinc-700/50 bg-zinc-800/80 p-2 text-white shadow-lg"
              >
                <RefreshCw size={18} />
              </button>
            )}
          ...
          {selectedId && !editingId && (
            <div className="absolute left-1/2 top-4 z-40 flex -translate-x-1/2 transform items-center gap-1 rounded-lg border border-zinc-700 bg-zinc-900 p-1 shadow-xl">
              <div className="group/color relative">
                <div
                  className="flex h-8 w-8 cursor-pointer items-center justify-center rounded hover:bg-zinc-800"
                  style={{ color: format.fill }}
                >
                  <Palette size={18} />
                  <div className="absolute bottom-1.5 h-1 w-4 rounded-full bg-current" />
                </div>
                <input
                  type="color"
              ...
              <div className="flex items-center gap-1 rounded border border-zinc-800 bg-zinc-950 px-1">
                <button
                  onClick={() =>
                    updateSelectedFormat(
                      "fontSize",
                      Math.max(8, format.fontSize - 2)
                    )
                  }
                ...
                <button
                  onClick={() =>
                    updateSelectedFormat(
                      "fontSize",
                      Math.min(120, format.fontSize + 2)
                    )
                  }
                  className="p-1 text-zinc-400 hover:text-white"
                >
              ...
              <button
                ...
                className={`rounded p-1.5 hover:bg-zinc-800 ${
                  format.bold ? "text-purple-400 bg-zinc-800" : "text-zinc-400"
                }`}
              >
                <Bold size={16} />
              </button>
        ...
        <button
          onClick={onNext}
          disabled={isLast}
          className="absolute right-4 z-40 rounded-full bg-black/50 p-4 text-white backdrop-blur transition hover:bg-black/80 disabled:opacity-0"
        >
          <ChevronRight size={24} />
        </button>

        {/* Bottom Main Toolbar */}
        <div className="absolute bottom-6 left-1/2 z-40 flex -translate-x-1/2 transform items-center gap-2 rounded-2xl border border-zinc-700 bg-zinc-900/90 p-2 shadow-2xl backdrop-blur">
          <button
            onClick={handleUndo}
            disabled={historyIndex <= 0}
            className="rounded-xl p-3 text-zinc-400 transition hover:bg-zinc-800 hover:text-white disabled:opacity-30 disabled:hover:bg-transparent"
            title="撤销"
          >
            <Undo size={20} />
          </button>
          ...
          <button
            ...
            className="bg-purple-600 hover:bg-purple-500 flex items-center gap-2 rounded-xl px-4 py-2 font-medium text-white shadow-lg shadow-purple-900/30 transition"
          >
            <Type size={18} />
            <span>添加文本</span>
          </button>
      ...
      <div className="flex justify-center gap-4 border-t border-zinc-800 bg-zinc-950 py-2 text-center text-xs text-zinc-500">
        <span className="flex items-center gap-1">
          <MousePointer2 size={12} /> 双击文本进行编辑
        </span>

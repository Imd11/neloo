...
const SlideThumbnails: React.FC<SlideThumbnailsProps> = ({
  ...
  return (
    <div className="custom-scrollbar flex w-24 flex-shrink-0 flex-col gap-4 overflow-y-auto border-r border-zinc-800 bg-zinc-950 p-4 pb-20 md:w-64">
        ...
        return (
          ...
          >
            ...
            >
              ...
              <div className="pointer-events-none absolute inset-0 flex items-end bg-gradient-to-t from-black/90 to-transparent p-2 opacity-0 opacity-70 hover:opacity-100">
            ...
            {!isDragging && (
              <>
                ...
                <button
                  ...
                  className="bg-purple-600 absolute -bottom-3 left-1/2 z-50 flex h-6 w-6 -translate-x-1/2 transform cursor-pointer items-center justify-center rounded-full text-white opacity-0 shadow-lg transition-all hover:scale-110 group-hover/thumbnail:opacity-100"
                  title="在此之后插入幻灯片"
                >
                  <Plus size={14} />
                </button>
      ...
      >
        <Plus size={24} />
        <span className="text-xs font-medium">新建幻灯片</span>
      </button>
    </div>
  );
};

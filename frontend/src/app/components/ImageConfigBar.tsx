...
export function ImageConfigBar({
  ...
  const ratioLabel =
    selectedRatio.value === "auto" ? t("image.auto") : selectedRatio.label;
  ...
  return (
    <div className="flex flex-wrap items-center justify-center gap-3">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            variant="outline"
            size="sm"
            className="gap-2 border-border bg-input-bg hover:bg-hover-bg"
          >
        ...
        <DropdownMenuContent
          align="start"
          className="min-w-32 border-border bg-popover"
        >
          {ratios.map((r) => (
            <DropdownMenuItem
              key={r.value}
            ...
            >
              ...
              {selectedRatio.value === r.value && <Check className="h-4 w-4" />}
            </DropdownMenuItem>
          ))}
        </DropdownMenuContent>
      </DropdownMenu>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent
          align="start"
          className="min-w-40 border-border bg-popover"
        >
          {resolutions.map((res) => (
            <DropdownMenuItem
              key={res.value}

...
function parseStructuredSuggestion(content: string): {
  ...
  if (jsonMatch) {
    try {
      ...
      if (
        parsed?.suggestion?.field &&
        parsed?.suggestion?.after !== undefined
      ) {
        return {
...
export function StepWizard({
  ...
  const singleStepSuggestionsRef = useRef<Record<StepId, SuggestionCard>>(
    {} as Record<StepId, SuggestionCard>
  );
  const [autoOptimizePausedByStep, setAutoOptimizePausedByStep] = useState<
    Record<StepId, boolean>
  >({} as Record<StepId, boolean>);
  const autoOptimizePausedByStepRef = useRef<Record<StepId, boolean>>(
    {} as Record<StepId, boolean>
  );

  const stepId = steps[currentStep].id as StepId;

  const [singleStepSuggestions, setSingleStepSuggestions] = useState<
    Record<StepId, SuggestionCard>
  >({} as Record<StepId, SuggestionCard>);
  const [experienceSuggestions, setExperienceSuggestions] = useState<
    Record<string, SuggestionCard>
  >({});
  ...
  const [skillsRecommendations, setSkillsRecommendations] =
    useState<SkillsRecommendationState>({
      status: "idle",
      recommendations: [],
    });
  const skillsRecommendationsRef = useRef<SkillsRecommendationState>({
    status: "idle",
    recommendations: [],
  });
  ...
  const moduleRecommendationsRef = useRef<{
    status: SuggestionStatus;
    recommendations: NewModuleRecommendation[];
    error?: string;
  }>({ status: "idle", recommendations: [] });
...
Choose the single field that is most worth optimizing and provide one suggestion.`;
  ...
  function buildPromptForListItem(
    ...
    return `Help me optimize item ${
      index + 1
    } in the resume ${label} section. Current item:
\`\`\`json
...
Choose the single field that is most worth optimizing. Allowed fields: ${allowed.join(
  ...
  function buildPromptForAdditionalItem(
    moduleType: AdditionalModuleType,
    index: number,
    item: unknown
  ): string {
...
Choose the single field that is most worth optimizing. Allowed fields: ${allowed.join(
  ...
  function rewriteAdditionalFieldPath(
    moduleType: AdditionalModuleType,
    itemId: string,
    field: string
  ): string | null {
  ...
  function getSinglePayload(step: StepId): unknown {
    ...
    if (step === "skills")
      return { skills: data.skills, languages: data.languages };
    if (step === "additional")
      return {
  ...
  async function generateSingleSuggestion(step: StepId, force = false) {
    ...
    if (!force && prev?.status === "loading" && prev.sourceHash === hash)
      return;
    if (
      !force &&
      prev?.status === "ready" &&
    ...
    try {
      const response = await sendMessage([
        { role: "user", content: buildPromptForSingle(step, payload) },
      ]);
...
4. Do not recommend skills the user already has.`;
  ...
  async function generateModuleRecommendations() {
    ...
    if (
      moduleRecommendationsRef.current.status === "loading" ||
  ...
  async function generateListSuggestions(
    step: "experience" | "education",
    force = false
  ) {
    ...
    const tasks: Array<() => Promise<void>> = items.map(
      (item, index) => async () => {
        ...
        if (!force) {
          if (existing?.status === "loading" && existing.sourceHash === hash)
            return;
        ...
        try {
          const response = await sendMessage([
            {
              role: "user",
              content: buildPromptForListItem(step, index, item),
            },
          ]);
          const parsed = parseStructuredSuggestion(response);
          if (!parsed.suggestion) {
          ...
          const suggestion = parsed.suggestion;
          const rewritten = rewriteListFieldPath(
            step,
            itemId,
            suggestion.field
          );
  ...
  async function generateAdditionalSuggestions(force = false) {
    const moduleTypes: AdditionalModuleType[] = [
      "projects",
      "certificates",
      "awards",
      "publications",
    ...
    const tasks: Array<() => Promise<void>> = allItems.map(
      ({ moduleType, item, index }) =>
        async () => {
          const itemId = item.id;
          ...
          try {
            const response = await sendMessage([
              {
                role: "user",
                content: buildPromptForAdditionalItem(moduleType, index, item),
              },
            ]);
            const parsed = parseStructuredSuggestion(response);
            if (!parsed.suggestion) {
            ...
            const suggestion = parsed.suggestion;
            const rewritten = rewriteAdditionalFieldPath(
              moduleType,
              itemId,
              suggestion.field
            );
  ...
  function regenerateCurrentStep(force = true) {
    ...
    if (stepId === "experience")
      void generateListSuggestions("experience", force);
    else if (stepId === "education")
      void generateListSuggestions("education", force);
  ...
  function ignoreSingleStepSuggestion(step: StepId) {
    setSingleStepSuggestions((prev) => ({
      ...prev,
      [step]: { ...prev[step], ignored: true },
    }));
  }

  function acceptListSuggestion(
    step: "experience" | "education",
    itemId: string
  ) {
    const cards =
      step === "experience" ? experienceSuggestions : educationSuggestions;
    ...
    setCards((prev) => ({
      ...prev,
      [itemId]: { ...prev[itemId], applied: true },
    }));
    setAutoOptimizePausedByStep((prev) => ({ ...prev, [step]: true }));
  }

  function ignoreListSuggestion(
    step: "experience" | "education",
    itemId: string
  ) {
    const setCards =
    ...
    setCards((prev) => ({
      ...prev,
      [itemId]: { ...prev[itemId], ignored: true },
    }));
  }
          ...
          {steps.map((step, index) => {
            ...
            return (
              <button
                ...
                disabled={isLocked}
                className={`step-nav-item ${isCompleted ? "completed" : ""} ${
                  isCurrent ? "current" : ""
                } ${isLocked ? "locked" : ""}`}
              >
          ...
          <div className="step-form-area">
            ...
            {currentStep === 4 && (
              <AdditionalStep
                data={data}
                onChange={onChange}
                prefetchedModuleRecommendations={moduleRecommendations}
              />
            )}
          ...
          {stepId !== "finalize" && (
            <>
              ...
              <div
                ...
                className={`step-resize-handle ${
                  resizingPanel === "ai" ? "active" : ""
                }`}
              />

              {/* AI Suggestions */}
              <aside
                className="step-ai-area"
                style={{ width: aiWidth }}
              >
                <div className="ai-card">
                  ...
                  <div className="ai-card-content">
                    {stepId === "experience" || stepId === "education" ? (
                                  ...
                                  <button
                                    ...
                                    onClick={() =>
                                      acceptListSuggestion(
                                        stepId as "experience" | "education",
                                        item.id
                                      )
                                    }
                                  >
                      ...
                      <div className="ai-suggestion-list">
                        {skillsRecommendations.status === "loading" ? (
                        ...
                        ) : skillsRecommendations.recommendations.length ===
                          0 ? (
                          <p className="ai-suggestion">
                            暂无推荐。完善简历内容后，AI
                            会分析您的背景并推荐相关技能。
                          ...
                          <>
                            ...
                            {skillsRecommendations.recommendations.map(
                              (rec, index) => {
                                if (rec.added) {
                                  return (
                                    <div
                                  ...
                                  >
                                    ...
                                    <div className="suggestion-actions">
                                      ...
                                      <button
                                        className="btn-ignore"
                                        onClick={() =>
                                          ignoreSkillRecommendation(index)
                                        }
                                      >
                                        忽略
                                      </button>
                      ...
                      <div className="ai-suggestion-list">
                        {(() => {
                          ...
                          const allItems = moduleTypes.flatMap((moduleType) =>
                            (data[moduleType] as { id: string }[]).map(
                              (item, index) => ({
                                moduleType,
                                item,
                                index,
                          ...
                          return allItems.map(
                            ({ moduleType, item, index, key }) => {
                              const card = additionalSuggestions[key] || {
                                status: "idle",
                              };
                              const moduleLabel =
                                ADDITIONAL_MODULE_LABELS[moduleType];
                                ...
                                >
                                  ...
                                  <div className="suggestion-actions">
                                    <button
                                      className="btn-accept"
                                      onClick={() =>
                                        acceptAdditionalSuggestion(key)
                                      }
                                    >
                                      ✅ 接受修改
                                    </button>
                                    <button
                                      className="btn-ignore"
                                      onClick={() =>
                                        ignoreAdditionalSuggestion(key)
                                      }
                                    >
                                      忽略
                                    </button>
                      ...
                      <div className="ai-suggestion-card">
                        ...
                        <div className="suggestion-before">
                          <span className="label">修改前：</span>
                          <span className="content">
                            {safeJSONStringify(singleCard.before || "(空)")}
                          </span>
                        </div>
                        <div className="suggestion-after">
                          <span className="label">修改后：</span>
                          <span className="content">
                            {safeJSONStringify(singleCard.after || "")}
                          </span>
                        </div>
                        <div className="suggestion-reason">
                          <span className="label">理由：</span>
                          <span className="content">
                            {singleCard.reason || singleCard.message || ""}
                          </span>
                        </div>
                        <div className="suggestion-actions">
                          <button
                            className="btn-accept"
                            onClick={() => acceptSingleStepSuggestion(stepId)}
                          >
                            ✅ 接受修改
                          </button>
                          <button
                            className="btn-ignore"
                            onClick={() => ignoreSingleStepSuggestion(stepId)}
                          >
                            忽略
                          </button>
                        </div>
                  ...
                  <button
                    ...
                    onClick={() => {
                      setAutoOptimizePausedByStep((prev) => ({
                        ...prev,
                        [stepId]: false,
                      }));
                    ...
                    disabled={
                      stepId === "experience"
                        ? Object.values(experienceSuggestions).some(
                            (c) => c.status === "loading"
                          )
                        : stepId === "education"
                        ? Object.values(educationSuggestions).some(
                            (c) => c.status === "loading"
                          )
                        : stepId === "additional"
                        ? Object.values(additionalSuggestions).some(
                            (c) => c.status === "loading"
                          )
                        : stepId === "skills"
                        ? skillsRecommendations.status === "loading"
  ...
  return (
    <div className="form-section">
      <div className="form-grid-2">
        ...
        <div className="form-group">
          <label className="form-label">
            职位/头衔 <span className="required">*</span>
          </label>
          <input
            type="text"
            className="form-input"
...
function ExperienceStep({
  ...
  const updateExperience = (
    index: number,
    updates: Partial<(typeof data.experience)[0]>
  ) => {
...
function EducationStep({
  ...
  const updateEducation = (
    index: number,
    updates: Partial<(typeof data.education)[0]>
  ) => {
...
function SkillsStep({
  ...
  const updateLanguage = (
    index: number,
    updates: Partial<(typeof data.languages)[0]>
  ) => {
      ...
      {data.languages.map((lang, i) => (
        ...
        >
          ...
          <select
            ...
            onChange={(e) =>
              updateLanguage(i, { levelNumber: parseInt(e.target.value) })
            }
          >
...
function FinalizeStep({ data }: { data: ResumeData }) {
  return (
    <div className="form-section finalize-section">
      ...
      <p className="finalize-description">点击"完成"进入编辑器，你可以：</p>
      ...
      <div className="finalize-summary">
        <div className="summary-item">
          ...
          <span className="summary-value">
            {data.personal.name || "未填写"}
          </span>
        </div>
        <div className="summary-item">
          <span className="summary-label">职位</span>
          <span className="summary-value">
            {data.personal.title || "未填写"}
          </span>
        </div>
        <div className="summary-item">

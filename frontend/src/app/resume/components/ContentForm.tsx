...
function PersonalInfoForm({
  ...
  const update = <K extends keyof PersonalInfo>(
    key: K,
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        <label className="mb-1 block text-xs text-gray-500">头像照片</label>
        <PhotoUpload
          value={data.photo || ""}
  ...
  return (
    <div className="space-y-2">
      ...
      {(items || []).map((item, i) => (
        <div
          key={item.id}
          className="space-y-1 rounded bg-gray-50 p-2 text-sm"
        >
      ...
      {(items || []).map((item, i) => (
        ...
        >
          ...
          <select
            ...
            onChange={(e) =>
              updateItem(i, { levelNumber: parseInt(e.target.value) })
            }

...
export function HipsterCVTemplate({ data, style, onDataChange }: Props) {
  ...
  const showCertificates =
    visibility?.certificates ?? (certificates?.length ?? 0) > 0;
  const showPublications =
    visibility?.publications ?? (publications?.length ?? 0) > 0;
      ...
      <header className="hipster-header">
        <div className="hipster-header-content">
          <div className="hipster-title-area">
            <span className="hipster-title-label">
      ...
      <div className="hipster-body">
            ...
            {languages.length > 0 && (
              <div className="hipster-language-bars">
                {languages.map((langItem, i) => (
                  <div
                    key={langItem.id || i}
                    className="hipster-language-bar"
                  >
                    <EditableText
                      tag="span"
        ...
        <main className="hipster-main">
          ...
          <section className="hipster-main-section">
            <h2 className="hipster-main-title">
            ...
            <div className="hipster-timeline">
              {experience.map((exp, i) => (
                <div
                  key={exp.id || i}
                  className="hipster-timeline-item"
                >
                  ...
                  <div className="hipster-timeline-content">
                    ...
                    <div className="hipster-timeline-company">
                      ...
                      />
                      {exp.location && (
                        <span className="hipster-location">📍</span>
                      )}
                    </div>
                    <EditableText
                      tag="p"
          ...
          {showCertificates && certificates && certificates.length > 0 && (
            <section className="hipster-main-section">
              <h2 className="hipster-main-title">
                {getLabel("certificates", lang)}
              </h2>
              <div className="hipster-timeline">
                {certificates.map((cert, i) => (
                  <div
                    key={cert.id || i}
                    className="hipster-timeline-item hipster-timeline-mini"
                  >
                    <EditableText
                      tag="div"
        ...
        <aside className="hipster-sidebar-right">
          ...
          {showHobbies && (
            <section className="hipster-section">
              <h2 className="hipster-section-title-right">
                {getLabel("hobbies", lang)}
              </h2>
              <div className="hipster-hobbies-grid">
                {data.hobbies?.slice(0, 4).map((hobby, i) => (
                  <div
                    key={hobby.id || i}
                    className="hipster-hobby-item"
                  >
          ...
          {showPublications && publications && publications.length > 0 && (
            <section className="hipster-section">
              <h2 className="hipster-section-title-right">
                {getLabel("publications", lang)}
              </h2>
              <div className="hipster-publications">
                {publications.map((pub, i) => (
                  <div
                    key={pub.id || i}
                    className="hipster-pub-item"
                  >
                    <EditableText
                      tag="span"
        ...
        <div className="hipster-footer-content">
          ...
          <span>
            ...
            <EditableText
              ...
              value={personal.address || ""}
              onChange={
                onDataChange ? (v) => updatePersonal("address", v) : undefined
              }
              placeholder="Address"
            />
          </span>

...
export async function generateImage(
  ...
  if (provider.provider === "gemini") {
    try {
      const response = await fetch(`${provider.baseUrl}/interactions`, {
        ...
        body: JSON.stringify(
          buildGeminiImageRequest(prompt, resolution, size, provider.model)
        ),
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Gemini image request failed: ${response.status} ${text}`
        );
      }
  ...
  try {
    ...
    if (!response.ok) {
      const text = await response.text();
      throw new Error(
        `${provider.provider} image request failed: ${response.status} ${text}`
      );
    }

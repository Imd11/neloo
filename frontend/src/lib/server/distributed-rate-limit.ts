...
export interface RateLimitStore {
  increment(keys: string[], limit: number, windowSeconds: number): Promise<RateLimitDecision>;
  reserve(key: string, units: number, limit: number, ttlSeconds: number): Promise<boolean>;
}

export class MemoryRateLimitStore implements RateLimitStore {
  private readonly values = new Map<string, { value: number; expiresAt: number }>();

  async increment(keys: string[], limit: number, windowSeconds: number): Promise<RateLimitDecision> {
    const now = Date.now();
    let allowed = true;
    let retryAfter = 1;
  ...
  async reserve(key: string, units: number, limit: number, ttlSeconds: number): Promise<boolean> {
    const now = Date.now();
    const existing = this.values.get(key);
    const entry = !existing || existing.expiresAt <= now
...
class RedisRateLimitStore implements RateLimitStore {
  ...
  async increment(keys: string[], limit: number, windowSeconds: number): Promise<RateLimitDecision> {
    ...
    return { allowed: Boolean(result[0]), retryAfter: Math.max(1, Number(result[1])) };
  }
  ...
  async reserve(key: string, units: number, limit: number, ttlSeconds: number): Promise<boolean> {
    const result = await this.client.eval(
      ...
      { keys: [key], arguments: [String(units), String(limit), String(ttlSeconds)] }
    );
...
export class DistributedRateLimiter {
  constructor(private readonly store: RateLimitStore, private readonly namespace = "neloo") {}
  ...
  async consume(
    ...
    const keys = [`${this.namespace}:window:${capability}:guest:${guestId}:${window}`];
    if (ipAddress) keys.push(`${this.namespace}:window:${capability}:ip:${ipAddress}:${window}`);
    return this.store.increment(keys, limit, windowSeconds);
  }

  reserveBudget(guestId: string, units: number, limit: number): Promise<boolean> {
    const day = new Date().toISOString().slice(0, 10);
    return this.store.reserve(`${this.namespace}:budget:${guestId}:${day}`, units, limit, 172800);
  }
}
  ...
  if (!limiterPromise) {
    limiterPromise = (async () => {
      ...
      if (!url) {
        ...
        console.warn("[usage-limits] Using single-process memory storage for local development only");
...
export function verifyGuestToken(token: string, secret: string | undefined, production: boolean): string {
  if (token.startsWith("local-dev:") && !production) return token.slice("local-dev:".length);
